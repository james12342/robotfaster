...
namespace taskt.Core.Automation.Commands
{
    ...
    public class HTTPExecuteRESTAPICommand : ScriptCommand
    {
        ...
        public string ExecuteRESTRequest(object sender)
        {
            ...
            //James add
            string requestRUL=String.Empty;
            if (v_AdvancedParameters.Rows.Count > 0)
            {
                targetEndpoint = targetEndpoint + "?";
            }

            var options = new RestClientOptions(v_BaseURL)
            {
                MaxTimeout = -1,
            };
            var client_J = new RestClient(options);

            foreach (DataRow rw in this.v_AdvancedParameters.Rows)
            {
                var paramName = rw.Field<string>("Parameter Name").ConvertToUserVariable(sender);
                var paramValue = rw.ItemArray[2].ToString();
                var paramType = rw.Field<string>("Parameter Type").ConvertToUserVariable(sender);
                // var contentType = rw.Field<string>("Content Type").ConvertToUserVariable(sender);
                // var contentType = "GetOrPost";
                //var param = new Parameter(paramName, paramValue, (ParameterType)Enum.Parse(typeof(ParameterType), paramType));
                ...
                {
                    targetEndpoint = targetEndpoint + "&" + paramName + "=" + paramValue;
                }

                //targetEndpoint = "/api/v3/order/page?ordersCreatedSince=2021-10-30&until=2021-12-20";
            }
            targetEndpoint = targetEndpoint.Replace("?&","?");
            var request_J = new RestRequest(targetEndpoint, method);
            ...
            foreach (DataRow rw in this.v_AdvancedParameters.Rows)
            {
                ...
                if (paramType == "HEADER")
                {
                    request_J.AddHeader(paramName, paramValue);
                }

                if (paramType == "JSON BODY")
                {
                    request_J.AddStringBody(paramValue, DataFormat.Json);
                }
            }

           

            //"/api/v3/order/page?ordersCreatedSince=2021-10-30&until=2021-12-20"
           
            //request_J.AddHeader("Authorization", "Bearer 70f45485-15a0-4350-9dd3-6a1ebc627fa7");
            RestResponse response_J = client_J.Execute(request_J);
            Console.WriteLine(response_J.Content);
            ...
            var content = response_J.Content;

            //add service response for tracking
            engine.ServiceResponses.Add(response_J);
